...
int second_max_elem()
{ // use <limit.h> and its function i.e INT_MIN for = max and 2ndmax
  ...
  for (int i = 0; i <= 7; i++)
  {
    ...
    else if (second_max < arr[i] && arr[i] != max_elem)
    { // this will run if if cond failed
      second_max = arr[i];
    }
  }
  // for(int i=0;i<=7;i++){
...
int modify_arr()
{ // arrays are passes by reference (values get changes in their list too)
  int num_size;
...
int diff_oddeven_arr()
{ // diff of sum_even indices and sum_odd indices
...
{ // rev part of arr in same org arr no temp arr
...
int countPairOfElem()
{ // count pair of sum of elem = to user input num
...
int search_elem()
{
  ...
  if (flag == 1)
    printf("elem is present at index %d\n", index);
  else
...
int find_elem()
{ // finding missing elem b/w a series of 1-100
